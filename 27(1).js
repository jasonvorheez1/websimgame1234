...
export async function executeAction(battle, actor, decision, parsed) {
  ...
  if (name.includes("rebelion's resolve".toLowerCase()) || name.includes('rebel') && name.includes('resolve') || (ability.type || '').toLowerCase() === 'ultimate') {
    ...
    // Formation Synergy: Bonus tenacity for the back row (where Leia usually is)
    const backRow = (actor.team === 'ally' ? battle.allies : battle.enemies).filter(u => !u.isDead && u.lane === 'back');
    backRow.forEach(u => u.applyStatus({ type: 'buff_tenacity', value: 20, duration: 8 }));
